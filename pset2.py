            ...
            for y in range(resultado.altura):
                ...
                for i in range(len(kernel)):
                    for j in range(len(kernel)):
                        cor = self.get_pixel((x - len(kernel) // 2 + j), (y - len(kernel) // 2 + i))
                        nova_cor += cor * kernel[i][j]
                resultado.set_pixel(x, y, cor)
        return resultado
...
if __name__ == '__main__':
    ...
    # QUESTÃO 02
    peixe = Imagem.carregar('imagens_teste/peixe.png')
    peixe_invertido = peixe.invertido()
    peixe_invertido.salvar('peixe_invertido.png')
